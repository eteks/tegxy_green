...
{
	// select image name
	$image=db_query("Select EVS_Id, EVS_ImgPath FROM ".TABLE_EVENTGALLERY." where EVS_Id='".$_GET["imageid"]."'") or die(db_error());
	if(db_num_rows($image)>0)
	{
		$image_row=db_fetch_array($image);
		$image_name=$image_row["EVS_ImgPath"];
		$check=db_query("Delete from ".TABLE_EVENTGALLERY." where EVS_Id='".$_GET["imageid"]."'") or die(db_error());
